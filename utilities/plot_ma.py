...
if __name__ == "__main__":
    ...
    df = (
        pd.read_csv(options.csv_file, compression="xz")
        if options.compressed
        else pd.read_csv(options.csv_file)
    )
    ...
    if options.plot_type[0] == "mean":
        df.drop(["Link", "Junction", "Junction Type"], axis="columns", inplace=True)
