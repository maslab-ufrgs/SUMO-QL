...
class BaseConfig(EmptyConfig):
    sumocfg: str | None = field(
        default=None, metadata=describe("Path to '.sumocfg' file. MANDATORY")
    )
    steps: int = field(
        default=60000,
        metadata=describe("Number of max simulation steps.", shorten=True),
    )
    demand: int = field(
        default=750, metadata=describe("Desired network demand.", shorten=True)
    )
    aw: int = field(
        default=1,
    ...
    nruns: int = field(
        default=1,
        metadata=describe("Number of multiple simulation runs.", shorten=True),
    )
    parallel: bool = field(
        default=False,
    ...
    observe_list: list[str] = field(
        default_factory=lambda: ["TravelTime"],
        metadata=describe("Parameters to collect data from.", rename="observe-list"),
    )

    collect_od_data: bool = field(
        default=False,
        metadata=describe(
            "Flag that indicates if simulation should collect data from od pairs",
            rename="collect-od",
        ),
    )
...
class GraphConfig(EmptyConfig):
    ...
    def main_group() -> _Group:
        return _Group(
            name="Graph Params (Used with communication)",
            description="Params used virtual graph",
        )

    file: str | None = field(
        default=None,
        metadata=describe(
            "Path to csv file that will be used as input for the virtual graph.",
            rename="vg-file",
            group=main_group(),
        ),
    )
    ...
    attributes: list[str] = field(
        ...
        metadata=describe(
            "List of attributes used to create the virtual graph. "
            "Attribute is given by the number of the column of the input csv.",
            rename="vg-attributes",
            group=main_group(),
        ),
    )
    ...
    labels: list[str | None] = field(
        ...
        metadata=describe(
            "List of attributes that will compose the label of each vertex in the virtual graph. "
            ...
            "i.e. aggregate by link or junction.",
            rename="vg-label",
            group=main_group(),
        ),
    )
    ...
    restrictions: list[str] = field(
        ...
        metadata=describe(
            "List of attributes that the vertices cannot share in order to create an edge in the virtual "
            "graph. Attribute is given by the number of the column of the input csv.",
            rename="vg-restrictions",
            group=main_group(),
        ),
    )
    ...
    threshold: float = field(
        default=0.0,
        metadata=describe(
            "Threshold used to create an edge in the virtual graph.",
            rename="vg-threshold",
            group=main_group(),
        ),
    )
    ...
    use_or: bool = field(
        default=False,
        metadata=describe(
            "Flag that indicates or logic instead of the and logic to create an edge between vertices given "
            "multiple attributes.",
            rename="use-or-logic",
            group=main_group(),
        ),
    ...
    measures: list[str] = field(
        default_factory=lambda: [],
        metadata=describe(
            "List of centrality measures to be taken of the virtual graph.",
            rename="centrality-measures",
            group=main_group(),
        ),
    )
    ...
    no_image: bool = field(
        default=False,
        metadata=describe(
            "Flag to indicate to the script not to generate the virtual graph image.",
            rename="no-image",
            group=main_group(),
        ),
    )
    ...
    raw: bool = field(
        default=False,
        metadata=describe(
            "Flag to indicate not to remove vertices with degree zero.",
            rename="raw-graph",
            group=main_group(),
        ),
    )
    ...
    giant: bool = field(
        default=False,
        metadata=describe(
            "Flag to indicate that only the giant component of the virtual graph should be "
            "presented in its image.",
            rename="giant-component",
            group=main_group(),
        ),
    )

    not_normalize: bool = field(
        default=False,
        metadata=describe(
            "Flag to indicate to the script not to normalize the input csv data to generate "
            "the virtual graph.",
            rename="vg-not-normalize",
            group=main_group(),
        ),
    )
    ...
    min_degree: int = field(
        default=0,
        metadata=describe(
            "Determines the minimum degree a vertex should have in order to be plotted in the virtual "
            "graph image.",
            rename="min-degree",
            group=main_group(),
        ),
    ...
    min_step: int = field(
        default=0,
        metadata=describe(
            "Determines the minimum step a vertex should have in order to be plotted in the virtual graph image.",
            rename="vg-min-step",
            group=main_group(),
        ),
    )

    vg_dict: str | None = field(
        default=None,
        metadata=describe(
            "Name of file containing python dictionary of virtual graph "
    ...
    interval: int = field(
        default=250,
        metadata=describe(
            "Amplitude of the timestep interval of the virtual graph "
...
class CommunicationConfig(EmptyConfig):
    ...
    def main_group() -> _Group:
        return _Group(
            name="Communication Params",
            description="Params used with C2I communication",
        )
    ...
    success_rate: float = field(
        default=0.0,
        metadata=describe(
            "Communication success rate.", rename="success-rate", group=main_group()
        ),
    )
    ...
    queue_size: int = field(
        default=30,
        metadata=describe(
            "CommDev queue size to store rewards.",
            rename="queue-size",
            group=main_group(),
        ),
    )
...
class LearningAgentConfig(BaseConfig):
    ...
    def main_group() -> _Group:
        return _Group(
            name="Learning Agent Params",
            description="Params used with any learning agent simulation",
        )

    wait_learn: int = field(
        default=3000,
        metadata=describe(
            "Time steps to wait before the learning starts.",
            rename="wait-learn",
            group=main_group(),
        ),
    )
    ...
    normalize_rewards: bool = field(
        default=False,
        metadata=describe(
            "Flag that indicates if rewards should be "
    ...
    collect_rewards: bool = field(
        default=False,
        metadata=describe(
            "Flag that indicates if rewards received should be "
            "collected to use them in normalizer in a posterior "
            "run.",
            rename="collect-rewards",
            group=main_group(),
        ),
    )

    toll_speed: float = field(
        default=-1,
        metadata=describe(
            "Speed threshold in which links should impose a toll on "
    ...
    toll_value: int = field(
        default=-1,
        metadata=describe(
            "Toll value to be added as penalty to emission. "
...
class QLConfig(LearningAgentConfig):
    ...
    def main_group() -> _Group:
        return _Group(
            name="Q-Learning Agent Params",
            description="Params used with Q-Learning simulation",
        )

    alpha: float = field(
        default=0.5, metadata=describe("Agent's learning rate.", group=main_group())
    )
    gamma: float = field(
        default=0.9,
        metadata=describe(
...
class PQLConfig(LearningAgentConfig):
    ...
    def main_group() -> _Group:
        return _Group(
            name="Pareto Q-Learning Agent Params",
            description="Params used with Pareto Q-Learning simulation",
        )
    ...
    objectives: list[str] = field(
        default_factory=lambda: ["TravelTime", "CO"],
        metadata=describe(
...
Config: TypeAlias = (
    QLConfig | PQLConfig | NonLearnerConfig | CommunicationConfig | GraphConfig
)


def add_fields(
    parser: argparse.ArgumentParser, config: Config
            ...
            case _:
                ...
                if group is None:
                    group = parser.add_argument_group(
                        config_group.name, config_group.description
                    )
